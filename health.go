...
type Health interface {
	...
	Results() (results map[string]types.Result, healthy bool)
	...
	DeregisterAll()
	// Views exposes health check views provided by this package.
	Views() *metrics.Views
}
...
func New(opts ...Option) Health {
	h := &health{
		...
		results:        make(map[string]types.Result, maxExpectedChecks),
...
func (h *health) Results() (results map[string]types.Result, healthy bool) {
	h.lock.RLock()
	defer h.lock.RUnlock()

	results = make(map[string]types.Result, len(h.results))
...
func (h *health) Views() *metrics.Views {
	return h.view
}

func (h *health) updateResult(
	name string, details interface{}, checkDuration time.Duration, err error, t time.Time) (result types.Result) {
	...
	h.stats.RecordStats(name, result, allHealthy(h.results))
